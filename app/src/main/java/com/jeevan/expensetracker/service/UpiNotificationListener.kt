...
class UpiNotificationListener : NotificationListenerService() {
    ...
    // FIX: a properly scoped coroutine scope tied to the service lifecycle,
    // with SupervisorJob so one failed coroutine doesn't cancel the others.
    private val serviceScope = CoroutineScope(Dispatchers.IO + SupervisorJob())

    override fun onDestroy() {
        super.onDestroy()
        // FIX: cancel all running coroutines when the service is destroyed
        serviceScope.cancel()
    }
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification?) {
        ...
        // FIX: use apply() instead of commit() — non-blocking
        sharedPref.edit()
        ...
        // FIX: use serviceScope instead of a bare CoroutineScope — tied to onDestroy(),
        // so coroutines are cancelled cleanly if the service stops.
        serviceScope.launch {
            try {
